...
switch ($action) {
    case 'check_id':
        ...
        try {
            ...
            } else { send_response(['success' => false, 'message' => 'La identificación ingresada no se encuentra registrada en nuestro sistema.']); }
    ...
    case 'send_code':
        ...
        try {
            ...
            $errorInfo = ($e instanceof \PHPMailer\PHPMailer\Exception) ? $mail->ErrorInfo : $e->getMessage();
            send_response(['success' => false, 'message' => 'No se pudo enviar el código de verificación. Info: ' . $errorInfo]);
        }
        break;
    ...
    case 'verify_code':
        ...
        $student_id = $_POST['student_id']; $code = trim($_POST['verification_code']);
        if (strlen($code) !== 6 || !ctype_digit($code)) { send_response(['success' => false, 'message' => 'El formato del código es inválido.']); }
        try {
            ...
            $stmt->execute([$student_id, $code]); $verification = $stmt->fetch();
            if (!$verification) { send_response(['success' => false, 'message' => 'El código es incorrecto o ya ha sido utilizado.']); }
            $now = new DateTime(); $expires = new DateTime($verification['expires_at']);
    ...
    case 'validate_certificate_code':
        ...
        try {
            ...
            if ($certificate) {
                $certificate['issue_date'] = (new DateTime($certificate['issue_date']))->format('d/m/Y');
                send_response(['success' => true, 'certificate' => $certificate]);
